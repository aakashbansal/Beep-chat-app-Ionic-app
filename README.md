...
**Firebase Cloud Functions** are the special functions ( or **triggers** ) that automatically gets invoked in response to a certain event. The event may be anything such as when the user **Signs Up** for the first time or the user **Logs in** or the **database** is **modified** in a way that a new data is added, deleted or modified. There may be many types of triggers that cause the **Cloud functions** to get called automatically by **Firebase**.
